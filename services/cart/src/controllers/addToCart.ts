...
import { getSessionId } from "@/lib/sessionService";
import { checkInventory, updateInventory } from "@/lib/inventoryService";
...
const addToCart = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    cartSessionId = await getSessionId(cartSessionId);
    res.setHeader("x-cart-session-id", cartSessionId);

    // Check inventory availability
    const inventoryData = await checkInventory(parsedBody.data.inventoryId);
    if (Number(inventoryData.quantity) < parsedBody.data.quantity) {
      return res.status(400).json({ message: "Inventory not available" });
    }

    // Add item to cart
    await addItemToCart(cartSessionId, parsedBody.data.productId, {
