...
import { createDoctorService } from "@/lib/createDoctorService";
import { DoctorCreateDTOSchema } from "@/schemas";
...
const createDoctor = async (
  ...
  try {
    ...
    const parsedBody = DoctorCreateDTOSchema.safeParse(req.body);
    // if validation fails, return an error
    if (!parsedBody.success) {
      return res.status(400).json({ errors: parsedBody.error.errors });
    }

    const doctor = await createDoctorService(parsedBody.data);
